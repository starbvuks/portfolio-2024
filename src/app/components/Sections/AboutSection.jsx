...
export default function AboutSection({ expanded }) {
  ...
  return (
    <div className={`font-esenka text-base md:text-xl transition-all duration-500 ${darkMode ? "text-[#ECECEC]" : "text-[#3c3c3c]"}`}>
      <div className="pl-4 md:pl-8 py-4 md:py-8 max-w-[90%] md:max-w-[60%] lg:max-w-[70%]">
        <p className={`${darkMode ? "text-[#A0A0A0]" : "text-[#646464]"} mb-4 md:mb-6`}>
          {data.about.bio}
        </p>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-8">
          {data.about.highlights.map((group, gi) => (
            <div key={gi}>
              <p className={`font-gambarino text-lg md:text-2xl ${darkMode ? "text-[#9a9a9a]" : "text-[#7a7a7a]"}`}>
                {group.label}
              </p>
